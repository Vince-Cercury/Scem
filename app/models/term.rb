class Term < ActiveRecord::Base
  ...
  def self.search_has_publisher_futur(search, page)
    paginate  :per_page => ENV['PER_PAGE'],
      :page => page,
  ...
  def self.search_has_no_publisher_futur(search, page, per_page)
    paginate  :per_page => per_page,
      :page => page,
      :include => [ :event],
      :conditions => ["events.name LIKE ? and terms.start > NOW() and events.id not in (select event_id from contributions where role='publisher' and event_id is not null)", "%#{search}%"],
      :order => 'start ASC'
  end
  ...
  def self.search_has_publisher_past(search, page)
    paginate  :per_page => ENV['PER_PAGE'],
      :page => page,
      :conditions => ['events.name LIKE ? and start <= NOW() ', "%#{search}%"],
      :joins => "inner join events on events.id = terms.event_id inner join contributions on contributions.event_id = events.id and contributions.role='publisher'",
  ...
  def self.search_has_publisher_futur_by_category(search, page, category_id)
    paginate  :per_page => ENV['PER_PAGE'],
      :page => page,
      :conditions => ['events.name LIKE ? and start >= NOW() and categories_events.category_id = ?', "%#{search}%", category_id],
  ...
  def self.search_has_no_publisher_futur_by_category(search, page, per_page, category_id)
    paginate  :per_page => per_page,
      :page => page,
      :include => [ {:event => :categories}],
      :conditions => ["events.name LIKE ? and start >= NOW() and categories.id = ?  and events.id not in (select event_id from contributions where role='publisher' and event_id is not null)", "%#{search}%", category_id],
      :order => 'start ASC'
  end
  ...
  def self.search_has_publisher_past_by_category(search, page, category_id)
    paginate  :per_page => ENV['PER_PAGE'],
      :page => page,
      :conditions => ['events.name LIKE ? and start <= NOW() and categories_events.category_id = ?', "%#{search}%",  category_id],
      :joins => "inner join events on events.id = terms.event_id inner join contributions on contributions.event_id = events.id and contributions.role='publisher' inner join categories_events on categories_events.event_id = events.id",
  ...
  def self.search_has_publisher_ended_by_category(search, page, category_id)
    paginate  :per_page => ENV['PER_PAGE'],
      :page => page,
      :joins => "inner join events on events.id = terms.event_id inner join contributions on contributions.event_id = events.id and contributions.role='publisher' inner join categories_events on categories_events.event_id = events.id",
      :conditions => ['events.name LIKE ? and end <= NOW() and categories_events.category_id = ?', "%#{search}%",  category_id],
      :order => 'end DESC',
      :group => 'terms.id'
  end
  ...
  def self.search_by_date_and_category(search, page, category_id, the_date)
    ...
    def self.search_has_publisher_by_date_and_category(search, page, category_id, the_date)
    #preparing 2 datetime dates from year, month, day
    #one at 00h00, the other at 23h59
    start_of_day = DateTime.new(the_date.year, the_date.month, the_date.day, 0, 0, 0).utc.to_s(:db)
    ...
    paginate  :per_page => ENV['PER_PAGE'],
      :page => page,
      :conditions => ['events.name LIKE ? and categories_events.category_id = ? and start < ? and end > ?', "%#{search}%", category_id, end_of_day, start_of_day],
      :joins => "inner join events on events.id = terms.event_id inner join contributions on contributions.event_id = events.id and contributions.role='publisher' inner join categories_events on categories_events.event_id = events.id",
      :order => 'start ASC',
      :group => 'terms.id'
  end

  def self.search_has_no_publisher_by_date_and_category(search, page, per_page, category_id, the_date)
    #preparing 2 datetime dates from year, month, day
    #one at 00h00, the other at 23h59
    start_of_day = DateTime.new(the_date.year, the_date.month, the_date.day, 0, 0, 0).utc.to_s(:db)
    ...
    end_of_day = DateTime.new(the_date.year, the_date.month, the_date.day, 23, 59, 59).utc.to_s(:db)
      ...
      :page => page,
      :include => [ {:event => :categories}],
      :conditions => ["events.name LIKE ? and categories.id = ? and start < ? and end > ?  and events.id not in (select event_id from contributions where role='publisher' and event_id is not null)", "%#{search}%", category_id, end_of_day, start_of_day],
      :order => 'start ASC'
  ...
  def self.count_occuring_in_the_day(category_id, the_date)
    ...
    count 'terms.id', :joins => "inner join events on events.id = terms.event_id inner join contributions on contributions.event_id = events.id and contributions.role='publisher' inner join categories_events on categories_events.event_id = events.id",
      :conditions => ['categories_events.category_id = ? and start < ? and end > ?', category_id, end_of_day, start_of_day],
    :distinct => true
  end
