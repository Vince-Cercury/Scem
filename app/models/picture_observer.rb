  ...
  def after_save(picture)
    ...
    if @moderation_state
      ...
      if picture.recently_activated? 
        ...
        PictureMailer.deliver_to_author_accepted_notification(@user_creator, picture, @controller)
        
        @list_sys_moderators.each do |user|
          puts "[picture-observer] deliver email to user (system moderator) with id=#{user.id}"
          PictureMailer.deliver_to_sys_moderators_accepted_notification(user, picture, @controller) if user.receive_picture_notification && user != @user_creator
        end
      end
    else
      puts "[picture-observer] picture moderation is OFF"
    ...
    if picture.recently_suspended?
      ...
      @list_sys_moderators.each do |user|
        puts "[picture-observer] deliver email to user (system moderator) with id=#{user.id}"
        PictureMailer.deliver_to_sys_moderators_suspended_notification(user, picture, @controller) if user.receive_picture_notification && user != @user_creator
      end
    end
