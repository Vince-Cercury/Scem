...
class OrganismObserver < ActiveRecord::Observer
  def after_create(organism)
    ...
    OrganismMailer.deliver_signup_notification_to_organism_admin_or_modo(@organism_creator, organism) if organism.pending? unless @organism_creator.email.nil?
    ...
    @system_admins_or_modo.each  do |user|
      OrganismMailer.deliver_signup_notification_to_system_admin_or_modo(user, organism) if organism.pending? unless user.email.nil?
    end
  end
  ...
  def after_save(organism)
    ...
    @organism_admins_or_modo.each  do |user|
      OrganismMailer.deliver_activation_to_organism_admin_or_modo(user, organism) if organism.recently_activated? unless user.email.nil?
    end

    #for each system_moderator and system_admin, send activation notification email
    @system_admins_or_modo = User.find(:all, :conditions => ["role = ? or role = ?", "admin", "moderator"] )
    @system_admins_or_modo.each  do |user|
      OrganismMailer.deliver_activation_to_system_admin_or_modo(user, organism) if organism.recently_activated? unless user.email.nil?
    end
  end
end
