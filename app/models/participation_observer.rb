class ParticipationObserver < ActiveRecord::Observer
  def after_create(participation)
    ...
    @list_moderators.each  do |admin_or_modo|
      ParticipationMailer.deliver_creation_notification(admin_or_modo, user, event, participation.term, participation.role)
    end
    ...
    @list_sys_moderators.each  do |admin_or_modo|
      ParticipationMailer.deliver_creation_notification_to_sys(admin_or_modo, user, event, participation.term, participation.role)
    end
  end
