  ...
  def after_create(organisms_user)
    organisms_user.reload
      ...
      @organism_admins_or_modo.each  do |admin_or_modo|
        OrganismsUserMailer.deliver_creation_notification(admin_or_modo, organisms_user.user, organisms_user.organism, organisms_user.role) if organisms_user.pending? unless user.email==""
      end
  end
