class OrganismsUserObserver < ActiveRecord::Observer
  def after_create(organisms_user)
    ...
    @organism_admins_or_modo.each  do |admin_or_modo|
      OrganismsUserMailer.deliver_creation_notification(admin_or_modo, organisms_user.user, organisms_user.organism, organisms_user.role) if organisms_user.pending?
    end
  end
  ...
  def after_save(organisms_user)
    ...
    OrganismsUserMailer.deliver_activation_notification(organisms_user.user, organisms_user.organism, organisms_user.role) if organisms_user.recently_activated?
