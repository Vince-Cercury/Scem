  ...
  module ByPassword
    ...
    def self.included(recipient)
      ...
      recipient.class_eval do
        ...
        validates_presence_of     :password,                   :if => :password_required? && :validate_password?
        validates_presence_of     :password_confirmation,      :if => :password_required? && :validate_password?
        validates_confirmation_of :password,                   :if => :password_required? && :validate_password?
        validates_length_of       :password, :within => 6..40, :if => :password_required? && :validate_password?
